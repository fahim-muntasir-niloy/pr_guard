...
function activate(context) {
    ...
    let oneClickPRDisposable = vscode.commands.registerCommand('pr-guard.oneClickPR', async () => {
        ...
        if (head === undefined)
            return;
        const panel = vscode.window.createWebviewPanel('prGuardOneClickPR', 'PR Guard One Click PR', vscode.ViewColumn.One, { enableScripts: true });
        panel.webview.html = (0, SimpleReportGenerator_1.getReportHtml)("", true);
...
async function checkCommand(cmd) {
    ...
    const options = shellStr ? { shell: shellStr } : {};
